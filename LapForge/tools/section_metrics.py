...
def _resolve_channel(canonical: str, channel_meta: dict) -> str | None:
    ...
    """
    from LapForge.channels import CHANNEL_SIGNATURES
...
def prepare_data(session_data: dict, options: dict | None = None) -> dict[str, Any]:
    ...
    if not sections:
        try:
            from LapForge.tools.section_generator import prepare_data as gen_data
    ...
    if ref_lats and ref_lons and len(ref_lats) == len(ref_lons) and len(ref_lats) >= 2:
        from LapForge.tools.section_generator import _haversine_distance
        cum = 0.0
        gps_dists = [0.0]
