...
def _store_token(user_key: str, token: dict) -> None:
    try:
        import keyring
        keyring.set_password("RaceDataAnalysis", f"oauth:{user_key}", json.dumps(token))
    except Exception:
...
def get_stored_token(user_key: str) -> dict | None:
    try:
        import keyring
        raw = keyring.get_password("RaceDataAnalysis", f"oauth:{user_key}")
        if raw:
...
def clear_stored_token(user_key: str) -> None:
    try:
        import keyring
        keyring.delete_password("RaceDataAnalysis", f"oauth:{user_key}")
    except Exception:
        pass
